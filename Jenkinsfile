pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'lucky7-cr/post-service'
    }
    stages {
        stage('Checkout') {
            steps {
                git branch: 'main', credentialsId: 'github-signin', url: 'https://github.com/KEA-Lucky7/MOABOA-BE-PostService.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Update Helm Values YAML') {
            steps {
                script {
                    ...
                    sh """
                    sed -i 's|tag: .*|tag: ${env.BUILD_ID}|' charts/post-service/values.yaml
                    """
                    withCredentials([usernamePassword(credentialsId: 'github-signin', usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD')]) {
                        ...
                        git add charts/post-service/values.yaml
                        git commit -m "[UPDATE] post service image tag ${env.BUILD_ID}"
